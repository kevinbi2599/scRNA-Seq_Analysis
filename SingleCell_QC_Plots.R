...
output_qcplots <- function(seurat_object){
  ...
  meta$DENS_logUMI_lognGene <- get_density(meta$log_nUMI, meta$nGene, n = 500)
...
PLOT3_percentrp_nUMI <- ggplot(meta) + geom_point(aes(log_nUMI, percent.rp, color = DENS_logUMI_percent.rp), size = 0.1) + scale_color_viridis() + theme(legend.position="none") + labs(title = paste("Ribosomal Genes"), x = "log10(nUMI)", y = "Fraction ribosomal molecules")

plot_grid(PLOT1_nGene_nUMI, PLOT2_percentmito_nUMI, PLOT3_percentrp_nUMI, ncol = 2)
}
